		...
		*/
		...
		if(rs.next()){
			...
			if(id.equals(rId) && passwd.equals(rPasswd)){ 
				...
				//37행: 해당 행의 name필드 값을 폼에서 입력받은 [ 이름으로 ]. --갱신하겠다
				sql = "UPDATE member SET name = ? WHERE id = ? " ; 
				pstmt = conn.prepareStatement(sql); // 다시, PreparedStatement 객체를 생성
				pstmt.setString(1, name); // 폼페이지에 전송된 [ 정해지지않는 값을 조건에 대입하도록 작성 ].
				pstmt.setString(2, id); // 폼페이지에 전송된 [ 정해지지않는 값을 조건에 대입하도록 작성 ].
				pstmt.executeUpdate(); //UPDATE 하기위해 사용.
				out.println("Member 테이블을 수정했습니다.<br>");// [ 수정 성공 ]하면 메시지를 출력.
				conn.commit();
			} //if문 블록 닫음
			else{

...
<jsp:useBean id="bbs" class="mvc.model.BoardDTO" scope="page" />
	...
	<%
		...
		if(userID == null){
			...
			if(bbs.getSubject() == null || bbs.getContent() == null){
				...
				//String id,String name,String subject, String content, int hit
				int result = bbsDAO.write(userID, bbs.getName(),bbs.getSubject() ,bbs.getContent());
				// 데이터베이스 오류인 경우
				if(result == -1){
					PrintWriter script = response.getWriter();
					script.println("<script>");

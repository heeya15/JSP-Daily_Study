...
<%  
	...
	*/
	...
	/* p,246 부분 59행에서 ~ 61행 추가작성
	  	MultipartRequest 객체타입의 getParameterNames()메소드를 작성한 후 
	  	Enumeration 객체 타입으로 반환.
	  60행: form에서 name = "productImage"으로 설정하여 productImage가 fname 변수에 저장.
	  61행: 서버에 실제로 업로드된 파일명을 반환. 즉, [ 파일명 ** 중복시 ** ] --> 변경된 파일명을 반환
	 */	
	Enumeration files = multi.getParameterNames();  // 59행은 : input type="file" 로 설정된 [ 요청 파라미터 이름 ]을 반환
